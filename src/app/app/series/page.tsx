...
export default function SeriesPage() {
  ...
  if (!isClient || (isLoading && allSeries.length === 0)) {
    return (
      <div>
        <h1 className="text-3xl font-bold mb-2 flex items-center"><Clapperboard className="mr-3 h-8 w-8 text-primary" /> Series</h1>
  ...
  if (mediaItems.length > 0 && allSeries.length === 0 && !isLoading) {
     return (
      <div className="flex flex-col items-center justify-center h-full text-center">
        ...
        <p className="text-muted-foreground mb-4">
          It seems there are no series in your playlists. Try adding a playlist with TV series.
        </p>
        <Link href="/app/settings" passHref>
          <Button>Go to Settings</Button>
        </Link>
  ...
  return (
    <div className="space-y-6">
      ...
      {allSeries.length > 0 && filteredSeries.length === 0 && !debouncedSearchTerm && !isLoading && (
         <div className="text-center py-10">
           <p className="text-muted-foreground">No series to display with current filters.</p>
         </div>
       )}
    </div>
  );
